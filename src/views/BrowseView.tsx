  ...
  return (
    <div className="flex min-h-full flex-col bg-air text-charcoal">
      <header className="px-4 pt-6 pb-4 text-center sm:px-6 sm:pt-8">
        ...
        <p className="mt-2 text-slate">A way in for the curious. No search box required.</p>
      </header>

      <main className="mx-auto w-full max-w-5xl flex-1 px-4 pb-12 sm:px-6">
        {/* Surprise me */}
        <section
          className="rounded-xl border border-mist bg-ice p-4 shadow-sm sm:p-6"
          aria-label="A random object"
        >
          <div className="flex items-center justify-between gap-4">
            ...
            <button
              ...
              className="shrink-0 rounded-full bg-berry px-5 py-2 text-sm font-bold text-ice transition-colors hover:bg-heather focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-berry"
            >
              Surprise me ↻
            </button>
          </div>
          <div className="mt-4 grid gap-4 sm:grid-cols-[240px_1fr] sm:items-stretch">
            ...
            <p className="self-center leading-relaxed text-slate">{surprise.description}</p>
          </div>
        </section>
        ...
        <section className="mt-10" aria-label="Themed trails">
          ...
          {trail && <p className="mt-3 text-sm text-slate">{trail.blurb}</p>}
